...
export default function EditInvoice() {
  ...
  const [itemsToDelete, setItemsToDelete] = useState([]); // Keep track of items removed from DB
    ...
    const fetchSessionAndInvoice = async () => {
      ...
      try {
        ...
        if (itemsData && itemsData.length > 0) {
          ...
          setItems([{ id: Date.now(), description: '', quantity: 1, price: 0, isNew: true }]);
        }
  ...
  const addItem = () => {
    setItems([...items, { id: Date.now(), description: '', quantity: 1, price: 0, isNew: true }]);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const itemsToUpdate = items.filter(i => !i.isNew).map(item => ({
        id: item.id,
        invoice_id: id,
        description: item.description,
        quantity: item.quantity,
      ...
      const itemsToInsert = items.filter(i => i.isNew).map(item => ({
        invoice_id: id,
        description: item.description,
        quantity: item.quantity,
        price: item.price
      ...
      if (itemsToUpdate.length > 0) {
        const { error: updateItemsError } = await supabase
          .from('invoice_items')
          .upsert(itemsToUpdate); // UPSERT works well if we provide the primary key 'id'
    ...
    <div style={{ minHeight: 'calc(100vh - 4.5rem)', backgroundColor: 'var(--bg-secondary)', padding: '2rem 1.5rem' }}>
      <div className="container" style={{ maxWidth: '800px' }}>
        ...
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate(`/invoices/${id}`)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)' }}>
            <ArrowLeft size={24} />
          </button>
          <div>
            <h1 className="font-serif heading-3" style={{ color: 'var(--text-primary)' }}>Edit Invoice</h1>
            <p style={{ color: 'var(--text-secondary)' }}>Update the details of your invoice below.</p>
          </div>
        </div>
