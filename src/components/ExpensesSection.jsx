...
import { ScanLine, Tags, TrendingUp, Cloud } from 'lucide-react';
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '4rem', alignItems: 'center' }}>
          ...
          >
            ...
            <motion.p variants={itemVariants} className="body-large mb-8">
              Snap a photo of any receipt, let OCR extract the details, and auto-categorize expenses across 24 categories.
            </motion.p>
            
            <motion.ul variants={containerVariants} style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {[
                { icon: <ScanLine size={20} color="var(--accent-blue)" />, text: "Receipt scanning with OCR extraction" },
                { icon: <Tags size={20} color="var(--accent-blue)" />, text: "Auto-categorize across 24 categories" },
                { icon: <TrendingUp size={20} color="var(--accent-blue)" />, text: "Monthly statistics and trend analysis" },
                { icon: <Cloud size={20} color="var(--accent-blue)" />, text: "Store receipt images securely in the cloud" },
              ].map((item, i) => (
          ...
          >
            <div style={{
              ...
              <div style={{ marginTop: '2rem', marginBottom: '1rem', padding: '0 0.5rem' }}>
                <h3 style={{ color: 'white', fontSize: '1.5rem', fontWeight: 700, marginBottom: '0.25rem' }}>Expenses</h3>
                <p style={{ color: '#a3a3a3', fontSize: '0.75rem' }}>Add expenses with receipt scanning</p>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '1.5rem' }}>
                ...
                <div style={{ backgroundColor: '#171717', padding: '1rem', borderRadius: '1rem', position: 'relative' }}>
                  <p style={{ color: 'white', fontWeight: 700 }}>₹1,77,154.06</p>
                  <p style={{ color: '#a3a3a3', fontSize: '0.7rem' }}>Total Amount</p>
              ...
              <h4 style={{ color: 'white', fontSize: '0.875rem', fontWeight: 600, marginBottom: '0.75rem', padding: '0 0.5rem' }}>Recent Expenses (9)</h4>
              
              {/* List Items */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', paddingBottom: '2rem' }}>
                {[
                  { name: "Kevin", amount: "₹45,000.00", tag: "Fuel", tagColor: "#f59e0b", bg: "rgba(245, 158, 11, 0.2)" },
                  { name: "Rajat Dalal", amount: "₹90,000.00", tag: "Professional Services", tagColor: "#a3a3a3", bg: "rgba(163, 163, 163, 0.2)" },
                  { name: "Mr Bean", amount: "₹15,000.00", tag: "Bank Fees", tagColor: "#a3a3a3", bg: "rgba(163, 163, 163, 0.2)" },
                  { name: "Sahil", amount: "₹1,000.00", tag: "Software & Subscriptions", tagColor: "#a855f7", bg: "rgba(168, 85, 247, 0.2)" }
                ].map((exp, i) => (
                  <div key={i} style={{ backgroundColor: '#171717', padding: '0.75rem', borderRadius: '0.75rem' }}>
                    <div className="flex justify-between items-center mb-1">
                      <span style={{ color: 'white', fontWeight: 500, fontSize: '0.875rem' }}>{exp.name}</span>
                      <span style={{ color: 'white', fontWeight: 700, fontSize: '0.875rem' }}>{exp.amount}</span>
                    </div>
                    <p style={{ color: '#a3a3a3', fontSize: '0.65rem', marginBottom: '0.5rem' }}>Apr 24</p>
                    <span style={{ backgroundColor: exp.bg, color: exp.tagColor, fontSize: '0.65rem', padding: '0.2rem 0.5rem', borderRadius: '0.25rem', fontWeight: 700 }}>{exp.tag}</span>
                  </div>
                ))}
              </div>
